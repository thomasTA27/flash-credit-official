...
async function loginUser() {
    ...
    const saltResponse = await fetch("http://localhost:8080/flash-credit/UserServlet/getSalt", {
        method: "POST",
    ...
    const response = await fetch("http://localhost:8080/flash-credit/UserServlet/signInUser", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (result.token) {
        ...
        window.location.href = "personal.html"; // Redirect to protected page
    } else {
...
form.addEventListener("submit", function(event) {
    ...
    console.log("Sign-in button clicked!"); // This should log correctly now
        ...
        console.log("yeowjhreow clicked!"); // This should log correctly now
